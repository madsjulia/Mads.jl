...
Gadfly.plot(Gadfly.layer(z=k, x=x, y=y, Gadfly.Geom.contour(levels=linspace(minimum(k),maximum(k),51))),Gadfly.layer(x=X[1,:], y=X[2,:], Geom.point, Gadfly.Theme(default_color=Meta.parse(Colors.Colorant, "red"), Gadfly.point_size=4pt)))
