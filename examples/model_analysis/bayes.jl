...
for w = (1000000, 1000, 1)
	...
	Mads.spaghettiplot(md, o, filename="bayes_init_w$(w)_spaghetti.png")
	...
	Gadfly.draw(Gadfly.PNG("bayes_init_w$(w)_o5.png", 6Gadfly.inch, 4Gadfly.inch), f)
end
...
for i = 1:3
	...
	for w = (1000000, 1000, 1)
		Mads.setobsweights!(md, w)
		mcmcchain = Mads.bayessampling(md; nsteps=10000, burnin=1000, thinning=1, seed=2016)
		Mads.scatterplotsamples(md, mcmcchain.value', "bayes_opt_$(p[i])_w$w.png")
		o = Mads.forward(md, mcmcchain.value)
		n = length(o)
		o = hcat(map(i->collect(values(o[i])), 1:n)...)'
		Mads.spaghettiplot(md, o, filename="bayes_opt_$(p[i])_w$(w)_spaghetti.png")
		@printf "O%-3s: Observation Weight %d StdDev %f -> `o5` prediction: min = %f max = %f\n" p[i] w 1/w min(o[:,5]...) max(o[:,5]...)
		f = Gadfly.plot(x=o[:,5], Gadfly.Guide.xlabel("o5"), Gadfly.Geom.histogram())
		Gadfly.draw(Gadfly.PNG("bayes_opt_$(p[i])_w$(w)_o5.png", 6Gadfly.inch, 4Gadfly.inch), f)
	end
end
