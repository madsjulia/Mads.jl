...
function spaghettiplot(madsdata::AbstractDict, dictarray::AbstractDict; seed::Integer=-1, rng::Union{Nothing,Random.AbstractRNG,DataType}=nothing, kw...)
	Mads.setseed(seed; rng=rng)
...
function plotseries(df::DataFrames.DataFrame, filename::AbstractString=""; names=string.(names(df)), kw...)
	plotseriesengine(Matrix(df), filename; names=names, kw...)
end
	...
	if combined
		...
		if !gall || code
			...
			c = [Gadfly.layer(x=xaxis, y=X[:,i],
				...
				Gadfly.Coord.Cartesian(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax),
				gm...,
				Gadfly.Theme(background_color=background_color, discrete_highlight_color=c->nothing, key_position=key_position, highlight_width=0Gadfly.pt, key_title_font_size=key_title_font_size, key_label_font_size=key_label_font_size, major_label_font_size=major_label_font_size, minor_label_font_size=minor_label_font_size)
			pS = Gadfly.plot(c...)
		else
