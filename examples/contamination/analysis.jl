...
Mads.plotmatches(md, forward_predictions, filename=rootname * "-w13aw20a-init-match.svg") # plot initial matches
run(`open w01-w13aw20a-init-match.svg`)

inverse_parameters, inverse_results = Mads.calibrate(md) # perform model calibration

inverse_predictions = Mads.forward(md, inverse_parameters) # execute forward model simulation based on calibrated values

Mads.plotmatches(md, inverse_predictions, filename=rootname * "-w13aw20a-calib-match.svg") # plot calibrated matches
run(`open w01-w13aw20a-calib-match.svg`)

# Sensitivity analysis: spaghetti plots based on prior parameter uncertainty ranges
paramvalues=Mads.parametersample(md, 10)
Mads.spaghettiplots(md, paramvalues, keyword="w13aw20a")
