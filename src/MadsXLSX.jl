...
function set_xlsx_formats(wf::XLSX.XLSXFile, dec_digits::Integer=4, exp_digits::Integer=2)
	set_xlsx_formats(wf.workbook, dec_digits, exp_digits)
end

function set_xlsx_formats(wb::XLSX.Workbook, dec_digits::Integer=4, exp_digits::Integer=2)
...
function apply_xlsx_format!(wf::XLSX.XLSXFile, format::XLSX.CellDataFormat=first(set_xlsx_formats(wf.workbook)))
	apply_xlsx_format(ws.workbook, format)
end

function apply_xlsx_format!(wb::XLSX.Workbook, format::XLSX.CellDataFormat=first(set_xlsx_formats(wb)))
	for ws in wb.sheets
		apply_xlsx_format(ws, format)
	end
end
...
function apply_xlsx_format(df::DataFrames.DataFrame, format::XLSX.CellDataFormat, cols::AbstractVector=names(df))
	return DataFrames.select(df, DataFrames.All(), cols .=> DataFrames.ByRow(x -> XLSX.CellValue(x, format)) .=> cols)
end