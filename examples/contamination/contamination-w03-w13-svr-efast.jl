...
for i=1:50
	pmodel = SVR.train(predictions[i,:], paramarray'; p=0.001, C=10000.0, gamma=10000.0);
	y_pr = SVR.predict(pmodel, paramarray');
	SVR.freemodel(pmodel)
	@show maximum(abs.(y_pr .- predictions[i,:]))
	# @assert maximum(abs.(y_pr .- predictions[i,:])) < 1e-4
