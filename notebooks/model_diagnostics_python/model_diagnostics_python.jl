...
cd(joinpath(Mads.dir, "notebooks", "model_diagnostics_python"))
...
"""
PyCall.py"polynomial_pycall"(1, 1, 1, 1)

function polynomial_pycall(parameters::AbstractVector{<:Real})::Vector{Float64}
	predictions::Vector{Float64} = Float64.(PyCall.py"polynomial_pycall"(parameters...))
	return predictions
end

function external_file_model(parameters::AbstractVector{<:Real})::Vector{Float64}
	return Float64.(parameters)
end

md::Dict{String,Any} = Mads.createproblem([1,1,1,2], [0,1.1,1.9,3.1,3.9,5], polynomial_pycall; paramkey=["a", "b", "c", "n"], paramdist=["Uniform(-10, 10)", "Uniform(-10, 10)", "Uniform(-5, 5)", "Uniform(0, 3)"], obsweight=[100,100,100,100,10,0], obstime=[0,1,2,3,4,5], obsdist=["Uniform(0, 1)", "Uniform(0, 2)", "Uniform(1, 3)", "Uniform(2, 4)", "Uniform(3, 5)", "Uniform(4, 6)"], problemname="model_diagnostics")
...
Mads.localsa(md; par=Float64.(collect(values(calibrated_parameters))), datafiles=false)
...
external_problem::Dict{String,Any} = Mads.createproblem("fileparam.txt", "fileobs.txt", external_file_model; problemname="external_files")
Mads.forward(external_problem)
