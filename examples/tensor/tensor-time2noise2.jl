...
NTFk.plot2d(permutedims(Wt, (1,3,2)), permutedims(Wt, (1,3,2)); xtitle="", ytitle="", wellnames=wellnames, keyword="noise-input", xmax=100, ymax=1., gm=[Gadfly.Guide.manual_color_key("Sources", ["S1", "S2", "S3", "Background"], NTFk.colors[1:4]), Gadfly.Theme(major_label_font_size=16Gadfly.pt, key_label_font_size=14Gadfly.pt, minor_label_font_size=12Gadfly.pt)])

Xn = convert(Array{Float32,3}, X .+ (randn(size(X)) * 20))
Xn[Xn.<0] .= 0
...
NTFk.plot2d(permutedims(Wet[4], (1,3,2))[:,:,[2,4,1,3]], permutedims(Wt, (1,3,2))[:,:,:]; xtitle="", ytitle="", wellnames=wellnames, keyword="noise-signals", xmax=100, ymax=1., gm=[Gadfly.Guide.manual_color_key("Sources", ["S1", "S2", "S3", "Background"], NTFk.colors[1:4]), Gadfly.Theme(major_label_font_size=16Gadfly.pt, key_label_font_size=14Gadfly.pt, minor_label_font_size=12Gadfly.pt)])
display(Ht)
display(Het[4][[2,4,1,3],:]/1000)

Xe = NMFk.mixmatchcompute(X, Wet[4], Het[4])
NTFk.plot2d(permutedims(Xe, (1,3,2))/1000, permutedims(Xn, (1,3,2))/1000; xtitle="", ytitle="", wellnames=wellnames, keyword="noise-species", xmax=100, ymin=0, ymax=1., gm=[Gadfly.Guide.manual_color_key("Species", ["A", "B", "C", "D"], NTFk.colors[1:4]), Gadfly.Theme(major_label_font_size=16Gadfly.pt, key_label_font_size=14Gadfly.pt, minor_label_font_size=12Gadfly.pt)])